    ...
    try {
      ...
      val nirSourceTiles =
        S3GeoTiffRDD[ProjectedExtent, LandsatKey, Tile](
          "landsat-pds",
          "L8/139/045/LC81390452014295LGN00/LC81390452014295LGN00_B5.TIF",
          uriToKey(3),
          options
        )
      ...
      val sourceTiles: RDD[(ProjectedExtent, MultibandTile)] = {
        sc.union(nirSourceTiles,redSourceTiles, greenSourceTiles, blueSourceTiles)
      ...
      GeoTiff(raster, metadata.crs).write("E://S3test.TIF")
    } finally {
      sc.stop()
    }
  }
